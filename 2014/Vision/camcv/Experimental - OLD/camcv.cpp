...
//new 
int nCount=0;
IplImage *py, *pu, *pv, *pupv, *pupv_big, *image,* dstImage,* dst32; //*pu_big, *pv_big,
	...
	if (pData)
	{
		...
		if (buffer->length)
		{
			...
			if (pData->pstate->graymode==0)
			{
				...
				cvThreshold(pu, pu, 118, 255, 1); //Was 110
				cvThreshold(pv, pv, 162, 255, 0); //Was 170
				cvAnd(pu,pv,pupv, NULL);
				cvNot(pupv,pupv);
				...
				cvShowImage("camcvWin", dst32);
				cvNormalize(dst32, dst32, 0.0, 1.0, CV_MINMAX, NULL);
				cvShowImage("norm_camcvWin", dst32);
...
int main(int argc, const char **argv)
{
	...
	dst32 = cvCreateImage(cvSize(w/2,h/2), IPL_DEPTH_32F, 1);
